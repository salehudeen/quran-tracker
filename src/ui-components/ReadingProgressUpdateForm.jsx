...
import { getReadingProgress } from "../graphql/queries";
import { updateReadingProgress } from "../graphql/mutations";
...
export default function ReadingProgressUpdateForm(props) {
  const {
    id: idProp,
    readingProgress: readingProgressModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = readingProgressRecord
      ? { ...initialValues, ...readingProgressRecord }
      : initialValues;
    setUserId(cleanValues.userId);
    setCurrentSurah(cleanValues.currentSurah);
    setCurrentAyah(cleanValues.currentAyah);
    setCurrentJuz(cleanValues.currentJuz);
    setCompletedSurahs(cleanValues.completedSurahs ?? []);
    setCurrentCompletedSurahsValue("");
    setCompletedJuzs(cleanValues.completedJuzs ?? []);
    setCurrentCompletedJuzsValue("");
    setTotalAyahsRead(cleanValues.totalAyahsRead);
    setLastUpdated(cleanValues.lastUpdated);
    setErrors({});
  };
  const [readingProgressRecord, setReadingProgressRecord] = React.useState(
    readingProgressModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getReadingProgress.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getReadingProgress
        : readingProgressModelProp;
      setReadingProgressRecord(record);
    };
    queryData();
  }, [idProp, readingProgressModelProp]);
  React.useEffect(resetStateValues, [readingProgressRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          currentAyah,
          currentJuz,
          completedSurahs: completedSurahs ?? null,
          completedJuzs: completedJuzs ?? null,
          totalAyahsRead,
          lastUpdated,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateReadingProgress.replaceAll("__typename", ""),
            variables: {
              input: {
                id: readingProgressRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ReadingProgressUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || readingProgressModelProp)}
