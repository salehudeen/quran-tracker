      ...
      try {
        ...
        if (client && userId) {
          ...
          const response = await client.graphql({
            query: `query GetReadingProgress($userId: ID!) {
              getReadingProgress(userId: $userId) {
                currentSurah
                currentAyah
                completedSurahs
                completedJuz
          ...
          // For demo, we'll just use default values
          setTimeout(() => {
            setSession({
              currentSurah: 1,
              currentAyah: 1,
  ...
  const markCurrentAyahAsRead = () => {
    ...
    juzData.forEach(juz => {
      ...
      if (
        ...
        !session.completedJuz.includes(juz.number)
      ) {
        newSession.completedJuz = [...session.completedJuz, juz.number];
      }
    });
    ...
    hizbData.forEach(hizb => {
      if (
        ...
        newSession.completedHizb = [...session.completedHizb, hizb.number];
      }
    });
    ...
    try {
      ...
      if (client) {
        await client.graphql({
          query: `mutation UpdateReadingProgress($input: ReadingProgressInput!) {
          ...
          variables: { 
            input: { 
              ...
              completedJuz: session.completedJuz,
              completedHizb: session.completedHizb,
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          <div className="bg-blue-50 p-3 rounded-lg">
            ...
            <div className="text-xs text-blue-700 mt-1">
              Completed: {session.completedJuz.length} of 30
            </div>
          </div>
